def dist_1d(a: int, b: int, L: int) -> int:
    '''
        Distance between two points on a periodic line with length L.
...
def dist_2d_lattice(u: tuple, v: tuple, L: int) -> int:
    '''
        Distance between two node on a periodic lattice graph with side length L.
...
def dist_2d_spacial(u: tuple, v: tuple, L: int) -> float:
    '''
        Distance in spacial coordinates between two nodes on a periodic lattice graph with side length L.
        ...
        Return:
        - r: spacial distance between points (-1 if error occured)
    '''
    # handle arguments having wrong types
    if type(u) != tuple or type(v) != tuple:
        print('ERROR: \nArguments u and v should be type tuple. You used type(u)={}, type(v)={}.\nBreak and return -1\
    ...
    r = (dist_1d(u[0], v[0], L)**2. + dist_1d(u[1], v[1], L)**2.)**.5
    return r


if __name__ == "__main__":
    print('dist_2d_spacial((0, 0), (3, 4), 10): {}'.format(dist_2d_spacial((0, 0), (3, 4), 10)))
