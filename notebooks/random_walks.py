...
def simple_2d_random_walk(G, L, max_steps, start_node=(0, 0),
                          use_random_starting_node=False):
    '''
    Perform a simple random walk on a given graph G, which should be based on
    a 2d lattice. In one step the random walker with equal probability
    traverses any of the incident edges of the current node to a neighboring
    node.
    ...
    Return:
    - path: list of edges the random walker traversed (format: [(node1, node2),
            (node2, node3)])
    '''
...
def lazy_2d_random_walk(G, L, max_steps, start_node=(0, 0),
                        use_random_starting_node=False):
    '''
    Perform a lazy random walk on a given graph G, which should be based on a
    2d lattice. In one step the random walker stays at the current node with
    probability 1/2 and with probability 1/2d_n traverses any of the d_n
    ...
    Return:
    - path: list of edges the lazy random walker traversed (format:
